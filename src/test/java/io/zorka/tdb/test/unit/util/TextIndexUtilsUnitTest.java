...
public class TextIndexUtilsUnitTest {
    ...
    public void testStringEscapeFn() {
        ...
        byte[] out = TextIndexUtils.escape(buf, 0, buf.length, TextIndexUtils.LIM_WAL);

        assertNotNull(out);

        for (int i = 0; i < out.length; i++) {
            if (out[i] == TextIndexUtils.ESC) {
                i++;
            } else {
                assertFalse(out[i] >= TextIndexUtils.ESC && out[i] < TextIndexUtils.LIM_WAL);
            }
        }

        byte[] dec = TextIndexUtils.unescape(out);
    ...
    public void testStringEscapeEscChar() {
        ...
        byte[] enc = TextIndexUtils.escape(buf);
        byte[] dec = TextIndexUtils.unescape(enc);

        for (int i = 0; i < enc.length; i++) {
            byte b = enc[i];
            if (b >= 0 && b < TextIndexUtils.LIM_WAL) {
